...
1. 只在用户明确要求判断、复盘、比较或制定策略时进入完整分析。
2. 把叙述拆分为已知事实、合理推测和关键未知，并按需读取参考文档。
3. 判断当前互动任务，再从状态、阶段、素材、表达、时机或反馈中选一个主要卡点。
4. 从承接、展示、调侃、推拉、筛选、投入测试、邀约或收线中只选一个主策略。
5. 给出一个明确首选和关键理由，收束为推进、观察或停止动作；后续按可见反馈反向校准。
...
3. 用户只问“怎么回”时，第一屏只给一个首选方案，拆成 1–4 个自然气泡并标明发送节奏，不展示 Mystery 术语或分析过程。
4. 抓住最新且最有互动价值的主线，不固定两句，不逐条复述全部碎片信息。
5. 成品至少加入一种新内容：个人判断、真实展示、轻调侃、角色感、选择感、共同画面、情绪张力、互动测试或自然钩子。
6. 普通感叹和玩笑不强塞邀约、见面或双人未来；自然窗口出现时再推进。需要演练时每轮只模拟一条回复并等待用户接话。
