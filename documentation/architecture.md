...
| Hermes runtime | 权威数据 | `database.py`、`repository.py`、`schema.sql` | schema v6、事务、人物/渠道隔离、事件、快照、结构化导入、个人记忆和任务队列 |
...
Feishu owner message
  ...
  -> owner + binding + route checks; select one profile runtime scope
  -> Gateway creates/loads the real profile session
  -> post_gateway_session -> bind real session ID + inject bounded context
...
1. 基础 `system` 前缀：Hermes 身份、工具与平台规则及会话元数据。当前关系 profile 不启用 `skills` toolset，因此这一运行面没有可调用 Skill 的 system 索引；关系 profile 同时关闭通用 `onboarding.first_message_intro`。
...
- schema v5 先创建/回填检索表，再按依赖顺序删除旧 `relationship_event_index_jobs` 和 `relationship_search_indexes`；schema v6 再为 `import_manifests` 增加结构化 manifest JSON 与 SHA256，并且只在列升级成功后记录 migration 6。迁移可重复执行，但真实执行仍需授权。
- 结构化导入必须同时匹配 source hash、manifest hash 和唯一活动人物。通用事件按 source hash、行号及事件类型生成稳定去重键；`wechat-agent-archive/v1` 事件使用 `conversation_id + msg anchor`，使同一消息在重叠导出文件之间仍然幂等。导出中标为 `unknown` 的作者只有在用户明确确认其为本机微信作者后才能映射为 `sent`；ASR、媒体描述和 `PARTIAL` 状态保留不确定性。归档源文件、manifest 及 SHA256 sidecar 不可覆盖。
- 历史错误 `sent` 不会被改写或删除。`correct-inferred-sent` 只为指定的自动推断事件追加 correction，并把实际发送状态保留为 unknown；recent context 排除已被 correction 失效的 `sent`，完整审计链仍可检索。
...
Hermes 0.20.4 派生运行时使用四文件原子补丁，并锁定每个源文件与补丁后 SHA256。profile route 确认后，hook、session 创建、历史读取、agent 执行、持久化和 cache 回基线都处于同一 `_profile_runtime_scope`；`pre_gateway_dispatch` 不创建 session，实际 session ID 只由 Gateway 创建后交给 `post_gateway_session`。Feishu adapter 按 session、sender、reply 和 thread 聚合图片及随后 2.5 秒内的兼容说明；每张新图或兼容文字重置窗口，不兼容上下文独立处理。

Gateway、turn context、Feishu adapter 和 Hermes 日志器的 INFO 记录只保留长度、数量、耗时与不可逆短哈希，不写用户正文、回复引用或视觉描述。日志目录为 `0700`，日志与 global/profile `.env` 为 `0600`；历史日志保留但收紧权限。
